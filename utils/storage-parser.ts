...
export function extractTitleInfo(
  pathname: string,
): { titleSlug: string; type: string; language: string } | null {
...
function parseProgressEntries(raw: Record<string, string>): AnimeProgress[] {
  ...
  return Array.from(progressMap.values()).filter(
    (e): e is AnimeProgress =>
      typeof e.titleSlug === "string" && e.titleSlug.length > 0,
  );
}

function parseParallelArrays<T>(
...
export function parseLocalStorage(
  ...
  return {
    ...
    history: parseParallelArrays<HistoryEntry>(raw, HISTORY_KEYS, [
      "url",
      "type",
      "name",
      "image",
    ...
    watchlist: parseParallelArrays<WatchlistEntry>(raw, WATCHLIST_KEYS, [
      "url",
      "name",
      "image",
    ]),
    favorites: parseParallelArrays<FavoriteEntry>(raw, FAVORITE_KEYS, [
      "url",
      "name",
      "image",
    ]),
